...
$(KCONFIG_TOOL_DIR)/mconf-idf $(KCONFIG_TOOL_DIR)/conf-idf: $(wildcard $(KCONFIG_TOOL_DIR)/*.c)
	$(eval MAKEFLAGS="" CC=$(HOSTCC) LD=$(HOSTLD) \
...
define RunConf
	$(eval mkdir -p $(BUILD_DIR_BASE)/include/config \
	cd $(BUILD_DIR_BASE); KCONFIG_AUTOHEADER=$(BUILD_DIR_BASE)/include/sdkconfig.h \
...
export KCONFIG_AUTOHEADER=$(BUILD_DIR_BASE)/include/sdkconfig.h
export COMPONENT_KCONFIGS
export KCONFIG_CONFIG=$(SDKCONFIG)
export COMPONENT_KCONFIGS_PROJBUILD
export IDF_CMAKE=n
...
ifdef BATCH_BUILD
	...
	$(PYTHON) $(PYTHON_GENERATE_CONFIG) $(IDF_PATH) $(BUILD_DIR_BASE) $(KCONFIG_TOOL_DIR) mconf-idf
	@echo "$(COMPONENT_KCONFIGS)"
...
ifneq ("$(wildcard $(SDKCONFIG_DEFAULTS))","")
	...
	$(PYTHON) $(PYTHON_GENERATE_CONFIG) $(IDF_PATH) $(BUILD_DIR_BASE) $(KCONFIG_TOOL_DIR) conf-idf --olddefconfig



# if neither defconfig or menuconfig are requested, use the GENCONFIG rule to
# ensure generated config files are up to date
$(SDKCONFIG_MAKEFILE) $(BUILD_DIR_BASE)/include/sdkconfig.h: $(KCONFIG_TOOL_DIR)/conf-idf $(SDKCONFIG) \
																$(COMPONENT_KCONFIGS) \
																$(COMPONENT_KCONFIGS_PROJBUILD) | $(call prereq_if_explicit,defconfig) \
																$(call prereq_if_explicit,menuconfig)
	$(summary) GENCONFIG
ifdef BATCH_BUILD  # can't prompt for new config values like on terminal
	$(PYTHON) $(PYTHON_GENERATE_CONFIG) $(IDF_PATH) $(BUILD_DIR_BASE) $(KCONFIG_TOOL_DIR) conf-idf --olddefconfig
endif
	$(PYTHON) $(PYTHON_GENERATE_CONFIG) $(IDF_PATH) $(BUILD_DIR_BASE) $(KCONFIG_TOOL_DIR) conf-idf --silentoldconfig
