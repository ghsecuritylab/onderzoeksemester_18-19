...
BaseType_t xTimeoutOccurred = pdFALSE;
	...
	vTaskSuspendAll();
	portENTER_CRITICAL(&pxEventBits->eventGroupMux);
	{
	...
	portEXIT_CRITICAL( &pxEventBits->eventGroupMux );
...
void vEventGroupDelete( EventGroupHandle_t xEventGroup )
{
	...
	portENTER_CRITICAL( &pxEventBits->eventGroupMux );
	{
		...
		#elif( ( configSUPPORT_DYNAMIC_ALLOCATION == 1 ) && ( configSUPPORT_STATIC_ALLOCATION == 1 ) )
		{
			...
			{
			    portEXIT_CRITICAL( &pxEventBits->eventGroupMux );   //Exit mux of event group before deleting it
			    vPortFree( pxEventBits );
			}
			else
			{
