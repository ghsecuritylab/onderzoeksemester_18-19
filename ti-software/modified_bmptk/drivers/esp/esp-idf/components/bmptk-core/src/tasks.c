...
TickType_t xReturn;
	...
	portENTER_CRITICAL_ISR(&xTickCountMutex);
	{
		xReturn = xTickCount;
//		vPortCPUReleaseMutex( &xTickCountMutex );
	}
	portEXIT_CRITICAL_ISR(&xTickCountMutex);
...
void vTaskSwitchContext( void )
{
	...
	else
	{
		...
		/*
		 We cannot do portENTER_CRITICAL_ISR(&xTaskQueueMutex); here because it saves the interrupt context to the task tcb, and we're
...
BaseType_t xReturn;
	...
	portENTER_CRITICAL(&xTickCountMutex);
	{
	...
	portEXIT_CRITICAL(&xTickCountMutex);
...
static void prvCheckTasksWaitingTermination( void )
{
	#if ( INCLUDE_vTaskDelete == 1 )
	{
		...
		while(uxTasksDeleted > ( UBaseType_t ) 0U )
		{
			...
			portEXIT_CRITICAL(&xTaskQueueMutex);	//Need to call deletion callbacks outside critical section
