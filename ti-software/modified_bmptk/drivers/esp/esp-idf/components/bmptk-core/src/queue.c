...
/*
 ToDo: The multicore implementation of this uses portENTER_CRITICAL etc to make sure the
 ...
 the portENTER_CRITICAL stuff to a lock + a scheduler suspend instead.
*/
...
Queue_t * const pxQueue = ( Queue_t * ) xQueue;
	...
	{
		portENTER_CRITICAL_ISR(&pxQueue->mux);
		if( ( pxQueue->uxMessagesWaiting < pxQueue->uxLength ) || ( xCopyPosition == queueOVERWRITE ) )
		{
		...
		portEXIT_CRITICAL_ISR(&pxQueue->mux);
	}
